...
ParallaxBackground::ParallaxBackground(IGame *game) : IGameObject(game, IGameObject::DRAW_ORDER_BACKGROUND, false) {
    this->setupLayer(3);
    this->setupLayer(2);
    this->setupLayer(1);
...
void ParallaxBackground::restore(const bool respawn) {

}

void ParallaxBackground::processEvents() {
...
void ParallaxBackground::setupLayer(const unsigned int layer) {
    ...
    std::vector<std::shared_ptr<SpriteEntity>> v;
