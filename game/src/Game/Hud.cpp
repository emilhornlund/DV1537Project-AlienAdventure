...
AA::Hud::Hud(CGL::IGame *game) : IGameObject(game, GameScene::DRAW_ORDER_HUD, false),
                        m_currentHealth(0),
                        m_currentCoins(0),
...
void AA::Hud::restore(const bool respawn) {
...
void AA::Hud::processEvents() {

}

void AA::Hud::update(const float dt) {
...
void AA::Hud::updateHealth(const unsigned health) {
...
void AA::Hud::updateCoins(const unsigned coins) {
...
void AA::Hud::setupCoinInitialDigit() {
    std::shared_ptr<CGL::SpriteEntity> entity = std::make_shared<CGL::SpriteEntity>();
    const auto& texture = this->getGame()->getTextureResourceHandler().load("Hud.png");
    entity->setTexture(texture);
...
void AA::Hud::setupCoinMultiplier() {
    std::shared_ptr<CGL::SpriteEntity> entity = std::make_shared<CGL::SpriteEntity>();
    const auto& texture = this->getGame()->getTextureResourceHandler().load("Hud.png");
    entity->setTexture(texture);
