...
CGL::Camera::Camera(const sf::FloatRect &rect) : m_view(std::make_shared<sf::View>(rect)),
                                                  m_bounds({0, 0, 0, 0}),
                                                  m_minCenter({0, 0}),
                                                  m_maxCenter({0, 0}),
                                                  m_moving({0, 0}),
...
CGL::Camera::~Camera() = default;

void CGL::Camera::setGlobalBounds(const sf::FloatRect &bounds) {
...
sf::FloatRect CGL::Camera::getGlobalBounds() const {
    return this->m_bounds;
}

void CGL::Camera::setCenter(const sf::Vector2f &center, bool force) {
...
sf::Vector2f CGL::Camera::getCenter() const {
    return this->m_view->getCenter();
}

sf::View &CGL::Camera::getView() const {
    return *this->m_view;
}

void CGL::Camera::update(const double &dt) {
    sf::Vector2f newCenter = this->m_view->getCenter();
